...
def spiralDiagonal(n):
    """
    Returns the sum of the diagonals of a clockwise n x n number spiral.
    n must be odd.
    """
    if (n%2 == 0):
        print "n must be odd!"
    ...
    if n == 1:
        ...
        layer = range((n-2)**2, n**2 + 1, n-1)[1:]
        return sum(layer) + spiralDiagonal(n-2)
    
# PROBLEM 29

def distinctPowers(minimum, maximum):
    """
    Returns the number of distinct terms generated by the integer
    combinations of a**b over minimum =< a,b => maximum.
    """
    numDict = {}
